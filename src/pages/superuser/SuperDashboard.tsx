...
import { getDatabaseSize} from '@/lib/api/databaseInfo';
...
export default function SuperDashboard() {
  ...
  useEffect(() => {
    ...
    // Auto-refresh every 30 seconds (keep for fallback, but real-time will handle most updates)
    ...
    const channel = supabase.channel('dashboard-updates')
      ...
      .on('postgres_changes', { event: 'DELETE', schema: 'public', table: 'projects' }, () => {
        setMetrics(prev => ({ ...prev, totalProjects: Math.max(0, prev.totalProjects - 1) }));
      })
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'logbook_entries' }, () => {
        setMetrics(prev => ({ ...prev, totalLogbooks: prev.totalLogbooks + 1 }));
      })
      .on('postgres_changes', { event: 'DELETE', schema: 'public', table: 'logbook_entries' }, () => {
        setMetrics(prev => ({ ...prev, totalLogbooks: Math.max(0, prev.totalLogbooks - 1) }));
      })
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'reviews' }, () => {
        setMetrics(prev => ({ ...prev, totalReviews: prev.totalReviews + 1 }));
      })
      .on('postgres_changes', { event: 'DELETE', schema: 'public', table: 'reviews' }, () => {
        setMetrics(prev => ({ ...prev, totalReviews: Math.max(0, prev.totalReviews - 1) }));
      })
      .subscribe();
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          <Card className="border-l-4 border-l-indigo-500 hover:shadow-md transition-shadow">
            <CardContent className="pt-6">
              ...
              <p className="text-3xl font-bold text-gray-900">{metrics.databaseSize < 1 ? '< 1' : Math.round(metrics.databaseSize)} MB</p>
